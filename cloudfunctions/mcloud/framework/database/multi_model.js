...
class MultiModel extends Model {
	...
	static async editForms(where, formName, objName, hasImageForms, mustPID = true) {
		...
		for (let k = 0; k < hasImageForms.length; k++) {
			for (let j in forms) {
				if ((forms[j].type == 'image' || forms[j].type == 'content' || forms[j].type == 'file')
	...
	static async groupCount(where, groupField, mustPID = true, retModel = '') {
		where = MultiModel._getWhere(where, mustPID);
		return await super.groupCount(where, groupField, retModel);
	}
