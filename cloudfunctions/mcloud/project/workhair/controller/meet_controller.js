...
class MeetController extends BaseProjectController {
	...
	async getMeetList() {
		...
		for (let k = 0; k < list.length; k++) {
			list[k].MEET_OBJ['level'] = Number(list[k].MEET_OBJ['level']);
			list[k].MEET_FEE = Number(dataUtil.fmtMoney(list[k].MEET_FEE / 100));
	...
	async getMyJoinDetail() {
		...
		if (join) {
			...
			join.JOIN_CANCEL_TIME = timeUtil.timestamp2Time(join.JOIN_CANCEL_TIME);
			join.JOIN_PAY_FEE = Number(dataUtil.fmtMoney(join.JOIN_PAY_FEE / 100));
		}
		return join;
	...
	async prepay() {
		...
		let service = new MeetService();
		return await service.prepay(this._userId, input.meetId, input.timeMark, input.formsList);
	}
	...
	async getMyPayFlowList() {
		...
		let result = await service.getPayFlowList(this._userId, input);
