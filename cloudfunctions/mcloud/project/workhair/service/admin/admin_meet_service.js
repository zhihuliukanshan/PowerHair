...
class AdminMeetService extends BaseProjectAdminService {
	...
	async checkinJoin(joinId, flag) {
		this.AppError('[理发]该功能暂不开放，如有需要请加作者微信：cclinux0730');
	}
	...
	async getJoinList({
		...
		let fields = 'JOIN_PAY_FEE,JOIN_PAY_STATUS,JOIN_CANCEL_TIME,JOIN_PAY_TIME,JOIN_IS_CHECKIN,JOIN_CHECKIN_TIME,JOIN_CODE,JOIN_ID,JOIN_REASON,JOIN_USER_ID,JOIN_MEET_ID,JOIN_MEET_TITLE,JOIN_MEET_DAY,JOIN_MEET_TIME_START,JOIN_MEET_TIME_END,JOIN_MEET_TIME_MARK,JOIN_FORMS,JOIN_STATUS,JOIN_ADD_TIME';
	...
	async getAdminMeetList({
		...
		let fields = 'MEET_FEE,MEET_CATE_ID,MEET_CATE_NAME,MEET_TITLE,MEET_STATUS,MEET_DAYS,MEET_ADD_TIME,MEET_EDIT_TIME,MEET_ORDER,MEET_VOUCH,MEET_QR';
	...
	/**取消 预约
	 */
	async cancelJoin(joinId, reason = '') {
		this.AppError('[理发]该功能暂不开放，如有需要请加作者微信：cclinux0730');
	}
